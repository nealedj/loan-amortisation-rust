...
fn main() {
    ...
    let f = |period_payment| {
        let schedule = build_schedule(principal, disbursal_date, first_capitalisation_date, first_payment_date, num_payments, daily_rate, period_payment, interest_method, false);
    ...
    let schedule = build_schedule(principal, disbursal_date, first_capitalisation_date, first_payment_date, num_payments, daily_rate, period_payment, interest_method, true);
...
fn calculate_rough_period_payment(principal: Decimal, annual_rate: Decimal, num_payments: u32) -> Decimal {
...
mod tests {
    ...
    fn test_build_schedule() {
        ...
        let schedule = build_schedule(principal, disbursal_date, first_capitalisation_date, first_payment_date, num_payments, daily_rate, period_payment, InterestMethod::ActualActual, true);

        assert_eq!(schedule.len(), 36);
        for payment in schedule.as_slice() {
            print_row(payment.month, payment.payment, payment.principal, payment.interest, payment.balance);
        }
